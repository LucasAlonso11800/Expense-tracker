...
export const fetchMovements = (type: StringParam, amount: StringParam, date: StringParam, categoryId: NumberParam, userId: number) => {
    return async (dispatch: Dispatch<HomePageAction>) => {
        dispatch({ type: HomePageActionTypes.FETCH_MOVEMENTS_BEGIN });
        try {
            const response: AxiosResponse = await axios.post(`${movementsURL}/get`, { type, amount, date, categoryId, userId });
